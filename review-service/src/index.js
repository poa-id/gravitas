...
function inline(s){return esc(s).replace(/\*\*([^*]+)\*\*/g,'<strong>$1</strong>').replace(/\*([^*]+)\*/g,'<em>$1</em>').replace(/\x60([^\x60]+)\x60/g,'<code>$1</code>').replace(new RegExp('\\\\[([^\\\\]]+)\\\\]\\\\([^\\\\s)]+\\\\)','g'),'$1').replace(new RegExp('\\\\[\\\\[([^\\\\]]+)\\\\]\\\\]','g'),'$1')}
