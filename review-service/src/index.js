...
function positionToolbar(range){if(matchMedia('(max-width:640px)').matches){toolbar.style.left='';toolbar.style.top='';return}const r=range.getBoundingClientRect();toolbar.style.left=Math.max(toolbar.offsetWidth/2+10,Math.min(innerWidth-toolbar.offsetWidth/2-10,r.left+r.width/2))+'px';toolbar.style.top=Math.max(48,r.top-8)+'px'}
...
function beginComment(kind){if(!selected)return;composing=kind;toolbar.innerHTML='<input id="mark-comment" maxlength="4000" placeholder="'+(kind==='question'?'What should the author consider?':'Leave a comment')+'" aria-label="Review comment"><button class="toolbar-cancel" id="mark-cancel" aria-label="Cancel comment" title="Cancel · Esc">×</button>';toolbar.style.display='flex';positionToolbar(selected.range);const input=document.getElementById('mark-comment');const cancel=()=>{composing=null;showActions()};document.getElementById('mark-cancel').onclick=cancel;input.focus();input.onkeydown=e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();if(input.value.trim())addMark(kind,input.value)}if(e.key==='Escape'){e.preventDefault();cancel()}}}
