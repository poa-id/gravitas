...
function renderMarkdown(md){const normalized=String(md??'').split(String.fromCharCode(13)).join('');const lines=normalized.split(String.fromCharCode(10)),out=[];let para=[],quote=[];const flush=()=>{if(para.length){out.push('<p>'+inline(para.join(' '))+'</p>');para=[]}if(quote.length){out.push('<blockquote>'+inline(quote.join(' '))+'</blockquote>');quote=[]}};for(const line of lines){const trimmed=line.trim();let level=0;while(level<3&&line.charAt(level)==='#')level++;if(level>0&&line.charAt(level)===' '){flush();out.push('<h'+level+'>'+inline(line.slice(level+1))+'</h'+level+'>');continue}if(trimmed.length>=3&&trimmed.split('').every(ch=>ch==='-')){flush();out.push('<hr>');continue}if(line.charAt(0)==='>'){if(para.length)flush();quote.push(line.slice(line.charAt(1)===' '?2:1));continue}if(!trimmed){flush();continue}if(quote.length)flush();para.push(trimmed)}flush();return out.join('')}
