...
const Editor = forwardRef<EditorHandle, EditorProps>(function Editor({ note, onNavOpen, onContentChange, onTitleChange, saveState='set', soundEnabled=false, pasteIntentEnabled=true, isScratch=false, isNewNote=false, onNewNoteDone, onPromote, onDeleteEntry }, ref) {
  ...
  useImperativeHandle(ref, () => ({
    appendEntry(text) {
      ...
      if (!v) return
      const n = v.state.doc.length
      v.dispatch({ changes: { from: n, insert: text }, selection: { anchor: n + text.length }, scrollIntoView: true })
    },
    insertAt(pos, text) { viewRef.current?.dispatch({ changes: { from: pos, insert: text } }) },
  ...
  const handleTitleCommit = () => {
    ...
    const current = viewRef.current?.state.doc.toString() ?? note.content
    onContentChange?.(current)
    setTimeout(() => onTitleChange?.(title), 900)
  }
  ...
  if (!pastePluginRef.current) pastePluginRef.current = createPasteIntentPlugin(s => setPasteBannerVisible(s.active), pasteIntentEnabledRef)
  ...
  useEffect(() => {
    ...
    const state = EditorState.create({
      ...
      extensions: [
        ...
        EditorView.domEventHandlers({ keydown(e) { if (!soundEnabledRef.current) return false; if (e.key.length === 1 || e.key === 'Enter' || e.key === 'Backspace' || e.key === 'Delete') playKeySound(); return false } }),
        EditorView.updateListener.of((u: ViewUpdate) => { if (u.docChanged) { const text = u.state.doc.toString(); setWords(countWords(text)); setChars(text.length); onContentChange?.(text) } }),
        EditorView.lineWrapping,
      ],
    })
    const v = new EditorView({ state, parent: editorRef.current })
    viewRef.current = v
    activeKeyRef.current = note.path
    const end = v.state.doc.length
    v.dispatch({ selection: { anchor: end }, scrollIntoView: false })
    requestAnimationFrame(() => { v.scrollDOM.scrollTop = v.scrollDOM.scrollHeight })
    v.focus()
    return () => v.destroy()
  }, [])
  ...
  useEffect(() => {
    const v = viewRef.current
    if (!v || note.path === activeKeyRef.current) return
    activeKeyRef.current = note.path
    setProgrammatic(true)
    v.dispatch({ changes: { from: 0, to: v.state.doc.length, insert: note.content }, scrollIntoView: false })
    setWords(countWords(note.content)); setChars(note.content.length)
    requestAnimationFrame(() => {
      const end = v.state.doc.length
      v.dispatch({ selection: { anchor: end }, scrollIntoView: false })
      v.scrollDOM.scrollTop = v.scrollDOM.scrollHeight
      requestAnimationFrame(() => { setProgrammatic(false); v.focus() })
    })
    if (isNewNote) {
      setEntering(true)
      const timer = setTimeout(() => setEntering(false), 300)
      if (!isScratch) {
        setTitleDraft(pathTitle(note.path) || note.title)
        setEditingTitle(true)
  ...
  const handleExportPdf = () => exportNoteAsPdf(displayTitle, viewRef.current?.state.doc.toString() ?? note.content)
  const mod = /Mac|iPhone|iPad/.test(navigator.platform) ? '⌘' : 'Ctrl'

  return (
    <div className={`gv-editor ${uiVisible || shortcutsOpen ? 'ui-visible' : ''}`} onMouseMove={showUI}>
      <div className="gv-topbar">
        <button className="gv-nav-toggle" onClick={onNavOpen} title="Navigate"><span/><span/><span/></button>
        <div className="gv-topbar-actions"><span className="gv-mode-tab active">Write</span><span className="gv-mode-tab">Read</span><span className="gv-mode-tab">Audit</span></div>
      </div>
      ...
      <div className={`gv-stage ${entering ? 'gv-entering' : ''}`}>
        <div className="gv-col">
          <div className="gv-header">
            {isScratch ? <div className="gv-scratch-label">scratch</div> : editingTitle ? <input ref={titleInputRef} className="gv-title gv-title-input" value={titleDraft} onChange={e => setTitleDraft(e.target.value)} onBlur={handleTitleCommit} onKeyDown={handleTitleKeyDown}/> : <h1 className={`gv-title ${onTitleChange ? 'gv-title-editable' : ''}`} onClick={handleTitleClick}>{displayTitle}</h1>}
      ...
      {shortcutsOpen && (
        <div className="gv-shortcuts">
          <span><kbd>{mod}N</kbd> new note</span><span><kbd>{mod}S</kbd> scratch</span><span><kbd>{mod}D</kbd> today</span><span><kbd>{mod}K</kbd> navigate</span><span><kbd>{mod + ','}</kbd> preferences</span><span><kbd>Esc</kbd> close</span>
        </div>
      )}
      <div className="gv-statusbar">
        <div className="gv-statusbar-stats"><span className="gv-stat"><span className={`gv-stat-dot ${saveState}`}/>{saveLabel}</span><span className="gv-stat">{words} words</span><span className="gv-stat">{chars} chars</span><span className="gv-stat">{readingTime(words)}</span></div>
        <div className="gv-statusbar-actions"><button className="gv-shortcuts-btn" onClick={() => setShortcutsOpen(v => !v)}>Shortcuts</button>{!isScratch && <button className="gv-export-pdf" onClick={handleExportPdf} title="Export this note as PDF">Export PDF</button>}</div>
      </div>
    </div>
  )
})
