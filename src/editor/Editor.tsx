...
const Editor=forwardRef<EditorHandle,EditorProps>(function Editor({note,onNavOpen,onContentChange,onTitleChange,saveState='set',soundEnabled=false,pasteIntentEnabled=true,isScratch=false,isNewNote=false,onNewNoteDone,onPromote,onDeleteEntry},ref){
 ...
 const [words,setWords]=useState(countWords(note.content)),[chars,setChars]=useState(note.content.length),[uiVisible,setUiVisible]=useState(false)
 ...
 const handleTitleClick=()=>{if(!onTitleChange)return;setTitleDraft(note.title);setEditingTitle(true);setTimeout(()=>titleInputRef.current?.select(),0)}
 const handleTitleCommit=()=>{setEditingTitle(false);const t=titleDraft.trim();if(t&&t!==note.title)onTitleChange?.(t)}
 ...
 return <div className={`gv-editor ${uiVisible?'ui-visible':''}`} onMouseMove={showUI}>
  ...
  <div className={`gv-stage ${entering?'gv-entering':''}`}><div className="gv-col"><div className="gv-header">{isScratch?<div className="gv-scratch-label">scratch</div>:editingTitle?<input ref={titleInputRef} className="gv-title gv-title-input" value={titleDraft} onChange={e=>setTitleDraft(e.target.value)} onBlur={handleTitleCommit} onKeyDown={handleTitleKeyDown}/>:<h1 className={`gv-title ${onTitleChange?'gv-title-editable':''}`} onClick={handleTitleClick}>{note.title}</h1>}{!isScratch&&<div className="gv-meta"><span>{note.meta.date}</span>{note.meta.tags.map(t=><span key={t} className="gv-tag">{t}</span>)}<span className="gv-type-badge">{note.meta.type}</span></div>}</div><div className="gv-cm-wrap" ref={editorRef}/></div></div>
  <div className="gv-statusbar"><div className="gv-statusbar-stats"><span className="gv-stat"><span className={`gv-stat-dot ${saveState}`}/>{saveLabel}</span><span className="gv-stat">{words} words</span><span className="gv-stat">{chars} chars</span><span className="gv-stat">{readingTime(words)}</span></div>{!isScratch&&<button className="gv-export-pdf" onClick={handleExportPdf} title="Export this note as PDF">Export PDF</button>}</div>
 </div>
})
