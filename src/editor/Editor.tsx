...
const Editor=forwardRef<EditorHandle,EditorProps>(function Editor({note,onNavOpen,onContentChange,onTitleChange,saveState='set',soundEnabled=false,pasteIntentEnabled=true,isScratch=false,isNewNote=false,onNewNoteDone,onPromote,onDeleteEntry},ref){
 ...
 useEffect(()=>{if(mode!=='read'||!readRef.current)return;const root=readRef.current;const frame=requestAnimationFrame(()=>paintReadReviews(root,openReviews));return()=>{cancelAnimationFrame(frame);root.querySelectorAll('.gv-review-overlay,.gv-review-margin-mark').forEach(mark=>mark.remove())}},[mode,readContent,reviews])
