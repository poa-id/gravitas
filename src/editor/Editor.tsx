...
const Editor=forwardRef<EditorHandle,EditorProps>(function Editor({note,onNavOpen,onContentChange,onTitleChange,saveState='set',soundEnabled=false,pasteIntentEnabled=true,isScratch=false,isNewNote=false,onNewNoteDone,onPromote,onDeleteEntry},ref){
 ...
 const changeMode=(next:EditorMode)=>{if(next==='audit')return;if(next==='read'){const current=viewRef.current?.state.doc.toString()??note.content;setReadContent(current);setShortcutsOpen(false)}setMode(next)}
 ...
 useEffect(()=>{const v=viewRef.current;if(!v||note.path===activeKeyRef.current)return;activeKeyRef.current=note.path;setMode('write');setReadContent(note.content);setProgrammatic(true);v.dispatch({changes:{from:0,to:v.state.doc.length,insert:note.content},scrollIntoView:false});setWords(countWords(note.content));setChars(note.content.length);requestAnimationFrame(()=>{const end=v.state.doc.length;v.dispatch({selection:{anchor:end},scrollIntoView:false});v.scrollDOM.scrollTop=v.scrollDOM.scrollHeight;requestAnimationFrame(()=>{setProgrammatic(false);v.focus()})});if(isNewNote){setEntering(true);const t=setTimeout(()=>setEntering(false),300);if(!isScratch){setTitleDraft(pathTitle(note.path)||note.title);setEditingTitle(true);setTimeout(()=>titleInputRef.current?.select(),80)}onNewNoteDone?.();return()=>clearTimeout(t)}},[note.path,note.content])
 useEffect(()=>{if(mode==='write')requestAnimationFrame(()=>viewRef.current?.focus())},[mode])
 useEffect(()=>{if(mode!=='read')return;const onKey=(e:KeyboardEvent)=>{if(e.key==='Escape'){e.preventDefault();setMode('write')}};window.addEventListener('keydown',onKey);return()=>window.removeEventListener('keydown',onKey)},[mode])
 ...
 return <div className={`gv-editor gv-mode-${mode} ${uiVisible||shortcutsOpen?'ui-visible':''}`} onMouseMove={showUI}>
  <div className="gv-topbar"><button className="gv-nav-toggle" onClick={onNavOpen} title="Navigate"><span/><span/><span/></button><div className="gv-topbar-actions"><button className={`gv-mode-tab ${mode==='write'?'active':''}`} onClick={()=>changeMode('write')}>Write</button><button className={`gv-mode-tab ${mode==='read'?'active':''}`} onClick={()=>changeMode('read')}>Read</button><button className="gv-mode-tab gv-mode-tab-disabled" title="Audit is coming next">Audit</button></div></div>
