...
const Editor=forwardRef<EditorHandle,EditorProps>(function Editor({note,onNavOpen,onContentChange,onTitleChange,saveState='set',soundEnabled=false,pasteIntentEnabled=true,isScratch=false,isNewNote=false,onNewNoteDone,onPromote,onDeleteEntry},ref){
 ...
 return <div className={`gv-editor gv-mode-${mode} ${uiVisible||shortcutsOpen?'ui-visible':''}`} onMouseMove={showUI}>
  ...
  {mode==='audit'&&<aside className="gv-audit-rail">{active?<><div className="gv-audit-count">{activeReview+1} / {openReviews.length}</div><div className={`gv-review-kind kind-${active.kind}`}>{kindLabel[active.kind]}</div><blockquote>{active.selectedText}</blockquote>{active.comment&&<p>{active.comment}</p>}<div className="gv-audit-actions"><button onClick={()=>goReview(activeReview-1)}>↑ Previous</button><button onClick={()=>goReview(activeReview+1)}>↓ Next</button><button className="gv-resolve" onClick={()=>void resolveActive()}>Resolve <kbd>{mod}↵</kbd></button></div></>:<div className="gv-audit-empty"><strong>Review clear.</strong><span>No unresolved review marks.</span></div>}<div className="gv-audit-signals"><span>{(currentContent().match(/^\s*\?\?/gm)||[]).length} open questions</span><span>Spellcheck {viewRef.current?.contentDOM.spellcheck?'on':'off'}</span></div></aside>}
