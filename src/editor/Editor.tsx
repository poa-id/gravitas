...
const Editor=forwardRef<EditorHandle,EditorProps>(function Editor({note,onNavOpen,onContentChange,onTitleChange,saveState='set',soundEnabled=false,pasteIntentEnabled=true,isScratch=false,isNewNote=false,onNewNoteDone,onPromote,onDeleteEntry},ref){
 ...
 const addReview=async(kind:ReviewKind,comment='')=>{if(!reviewSelection.trim())return;const review=makeReview(kind,reviewSelection,readContent,comment);const next=[...reviews,review];setReviews(next);clearReviewSelection();await saveReviews(note.path,next)}
 const beginComment=()=>{setCommenting(true);setTimeout(()=>commentInputRef.current?.focus(),0)}
 const captureReadSelection=()=>{if(mode!=='read'||commenting)return;requestAnimationFrame(()=>{const selection=window.getSelection();const text=selection?.toString().trim()??'';if(!text||!selection?.rangeCount){setReviewPoint(null);setReviewSelection('');return}const range=selection.getRangeAt(0);if(!readRef.current?.contains(range.commonAncestorContainer)){setReviewPoint(null);setReviewSelection('');return}const rect=range.getBoundingClientRect();if(!rect.width&&!rect.height)return;setReviewSelection(text);setReviewPoint({x:Math.min(window.innerWidth-180,rect.left+rect.width/2),y:Math.max(46,rect.top-8)})})}
